...
namespace Vinchuca.REPL
{
    class AddNodeCommand : Command
    {
        ...
        public AddNodeCommand(Agent agent)
        ...
        public override int Invoke(IEnumerable<string> args)
        {
            try
            {
                ...
                if (extra.Count == 0)
                {
                    Console.Error.WriteLine("commands: Missing required argument `endpoint`.");
                    Console.Error.WriteLine("commands: Use `help add-node` for details.");
                    return 1;
                }
